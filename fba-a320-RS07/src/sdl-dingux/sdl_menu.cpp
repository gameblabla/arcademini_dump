...
void gui_Flip()
{
	...
	//SDL_BlitSurface(menuSurface, 0, rl_screen, &dstrect);
	SDL_SoftStretch(menuSurface, NULL, rl_screen, 0);
	SDL_Flip(rl_screen);
}
...
uint32_t pressed_b[5] = { 0, 0, 0, 0, 0 };
#endif
	...
	while(!done) {
	...
	#ifdef TARGET_ARCADEMINI
		{
			...
			for(uint8_t joynumb = 0;joynumb<2;joynumb++)
			{
				...
				if (SDL_JoystickGetButton(joys[joynumb], 2))
				{
					if(mi->itemOnA != NULL) (*mi->itemOnA)();
					SDL_Delay(128);
				}
				
				if (SDL_JoystickGetButton(joys[joynumb], 1))
				{
					SDL_Delay(128);
					while(1)
					{
