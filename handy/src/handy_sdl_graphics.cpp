...
int handy_sdl_video_early_setup(int surfacewidth, int surfaceheight, int sdl_bpp_flag, int videoflags)
{
	...
	#ifdef TARGET_RS97
	real_surface = SDL_SetVideoMode(320, 480, 16, videoflags);
	mainSurface = SDL_CreateRGBSurface(SDL_SWSURFACE, surfacewidth, surfaceheight, 16, 0,0,0,0);
	#else
...
UBYTE *handy_sdl_display_callback(ULONG objref)
{
    ...
    gui_ShowFPS();
    #ifdef TARGET_RS97
    SDL_SoftStretch(mainSurface, NULL, real_surface, NULL);
...
{/*
#ifdef DINGUX
    Uint8 *dst_offset = (Uint8 *)dst->pixels + (dst->w - 320) + (dst->h - (LynxHeight*2)) * dst->w; 
...
{
	/* Disabled filtering as it's not implemented properly for vertical games like Raiden */
    /*if( filter >= 1 )
    {
    ...
    else*/
    {
        ...
        //if (LynxScale == 1)
        {
            ...
            if (mpLynx->CartGetRotate() == 0)
            {
				...
				{
					case 320:
						upscale_320x240((Uint32 *)HandyBuffer->pixels, (Uint32 *)mainSurface->pixels, LynxWidth, LynxHeight);
						break;
					case 400:
						upscale_400x240((Uint32 *)HandyBuffer->pixels, (Uint32 *)mainSurface->pixels, LynxWidth, LynxHeight);
						break;
					case 480:
						upscale_480x272((Uint32 *)HandyBuffer->pixels, (Uint32 *)mainSurface->pixels, LynxWidth, LynxHeight);
						break;
				}
			}
			else
