...
bool8_32 S9xDeinitUpdate (int Width, int Height, bool8_32)
{
	...
	switch (mMenuOptions.fullScreen)
	{
		case 0:
		{
			...
			uint8_t *src = (uint8_t*) IntermediateScreen, *dst = (uint8_t*) sal_RS97VideoGetBuffer()
		...
		case 3:
			if (PAL) {
				upscale_256x240_to_480x272((uint32_t*) sal_RS97VideoGetBuffer(), (uint32_t*) IntermediateScreen);
			} else {
				upscale_256x224_to_480x272((uint32_t*) sal_RS97VideoGetBuffer(), (uint32_t*) IntermediateScreen);
			}
		break;
	}
...
uint32 S9xReadJoypad (int which1)
{
	...
	joy_s9x = sal_InputPoll(which1);
		
	if (joy_s9x & SAL_INPUT_MENU)
	{
		mEnterMenu = 1;		
		return val;
	}

	if (joy_s9x & SAL_INPUT_Y) val |= SNES_Y_MASK;
	if (joy_s9x & SAL_INPUT_A) val |= SNES_A_MASK;
	if (joy_s9x & SAL_INPUT_B) val |= SNES_B_MASK;
	if (joy_s9x & SAL_INPUT_X) val |= SNES_X_MASK;
			
	if (joy_s9x & SAL_INPUT_UP) 	val |= SNES_UP_MASK;
	if (joy_s9x & SAL_INPUT_DOWN) 	val |= SNES_DOWN_MASK;
	if (joy_s9x & SAL_INPUT_LEFT) 	val |= SNES_LEFT_MASK;
	if (joy_s9x & SAL_INPUT_RIGHT)	val |= SNES_RIGHT_MASK;
	if (joy_s9x & SAL_INPUT_START) 	val |= SNES_START_MASK;
	if (joy_s9x & SAL_INPUT_SELECT) val |= SNES_SELECT_MASK;
	if (joy_s9x & SAL_INPUT_L) 		val |= SNES_TL_MASK;
	if (joy_s9x & SAL_INPUT_R) 		val |= SNES_TR_MASK;
