...
extern SDL_Joystick *s_Joysticks[4];
...
static void UpdatePhysicalInput()
{
    ...
    /*printf("Axis %d\n", SDL_JoystickGetAxis(s_Joysticks[0], 0));
	printf("Axis 2 %d\n", SDL_JoystickGetAxis(s_Joysticks[0], 1));
	printf("Button 1 %d\n", SDL_JoystickGetButton(s_Joysticks[0], 1));
	printf("Button 2 %d\n", SDL_JoystickGetButton(s_Joysticks[0], 2));
	printf("Start %d\n", SDL_JoystickGetButton(s_Joysticks[0], 9));
	printf("Select %d\n", SDL_JoystickGetButton(s_Joysticks[0], 8));*/
	
	//JS |= (1 << 1) << (0 << 3);
	
	if (SDL_JoystickGetButton(s_Joysticks[0], 9) && SDL_JoystickGetButton(s_Joysticks[0], 8))
	{
		MenuRequested = true;
	}
...
static void UpdateGamepad(void) {
	...
	/* HAAACCCKS, For Arcade Mini obviously */
	if (SDL_JoystickGetButton(s_Joysticks[0], 1)) JS += 1;
	if (SDL_JoystickGetButton(s_Joysticks[0], 2)) JS += 2;
	if (SDL_JoystickGetButton(s_Joysticks[0], 8)) JS += 4;
	if (SDL_JoystickGetButton(s_Joysticks[0], 9)) JS += 8;
	if (SDL_JoystickGetAxis(s_Joysticks[0], 0) < -500) JS += 64;
	else if (SDL_JoystickGetAxis(s_Joysticks[0], 0) > 500) JS += 128;
	if (SDL_JoystickGetAxis(s_Joysticks[0], 1) < -500) JS += 16;
	else if (SDL_JoystickGetAxis(s_Joysticks[0], 1) > 500) JS += 32;
	
	if (SDL_JoystickGetButton(s_Joysticks[1], 1)) JS += 1 * 256;
	if (SDL_JoystickGetButton(s_Joysticks[1], 2)) JS += 2 * 256;
	if (SDL_JoystickGetButton(s_Joysticks[1], 8)) JS += 4 * 256;
	if (SDL_JoystickGetButton(s_Joysticks[1], 9)) JS += 8 * 256;
	if (SDL_JoystickGetAxis(s_Joysticks[1], 0) < -500) JS += 64 * 256;
	else if (SDL_JoystickGetAxis(s_Joysticks[1], 0) > 500) JS += 128 * 256;
	if (SDL_JoystickGetAxis(s_Joysticks[1], 1) < -500) JS += 16 * 256;
	else if (SDL_JoystickGetAxis(s_Joysticks[1], 1) > 500) JS += 32 * 256;
