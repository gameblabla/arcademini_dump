...
void hwsprites::render(const uint8_t priority)
{
    ...
    for (uint16_t data = 0; data < SPRITE_RAM_SIZE; data += 8) 
    {
        ...
        for (y = top; y != ytarget; y += ydelta)
        {
            ...
            /* xpos >= 0 && xpos < config.s16_width is here to prevent sprites being drawn in a glitchy way - Gameblabla */
            if (y >= 0 && y < config.s16_height && xpos >= 0 && xpos < config.s16_width)
            {
