...
bool V_DoModeSetup (int width, int height, int bits)
{
	...
	/* Forced to 1.0f because it is not saved properly to the config file */
	screen->SetGamma (1.0f);
...
SDLFB::SDLFB (int width, int height, bool fullscreen): DFrameBuffer (width, height)
{
	...
	rl_scr = SDL_SetVideoMode (width, height, 16, SDL_SWSURFACE|SDL_HWPALETTE);
	Screen = SDL_CreateRGBSurface(SDL_SWSURFACE|SDL_HWPALETTE, width, height, 8, 0,0,0,0);
...
void SDLFB::Update ()
{
	...
	SDL_BlitSurface(Screen, NULL, rl_scr, NULL);
	SDL_Flip (rl_scr);
